...
const SERVICES: {
...
}[] = [
  {
    ...
    timeline: '4–8 weeks',
    featured: true,
    cardAccent: 'from-primary/8 via-transparent to-transparent',
    iconAccent: 'bg-primary/10 text-primary group-hover:bg-primary/20',
  },
  {
    Icon: ShoppingBag,
    key: 'ecommerce',
    ...
    timeline: '6–12 weeks',
    featured: false,
    cardAccent: 'from-emerald-500/8 via-transparent to-transparent',
    iconAccent: 'bg-emerald-500/10 text-emerald-400 group-hover:bg-emerald-500/20',
  },
  {
    Icon: Server,
    key: 'backend',
    tech: ['Node.js', 'Express', 'GraphQL', 'PostgreSQL', 'Redis'],
    timeline: '3–8 weeks',
    featured: false,
    cardAccent: 'from-blue-500/8 via-transparent to-transparent',
    iconAccent: 'bg-blue-500/10 text-blue-400 group-hover:bg-blue-500/20',
  },
  {
    Icon: Layers,
    key: 'design',
    ...
    timeline: '2–4 weeks',
    featured: false,
    cardAccent: 'from-pink-500/8 via-transparent to-transparent',
    iconAccent: 'bg-pink-500/10 text-pink-400 group-hover:bg-pink-500/20',
  },
  {
    Icon: Zap,
    key: 'performance',
    tech: ['Lighthouse', 'GA4', 'Search Console', 'Next.js'],
    timeline: '2–4 weeks',
    featured: false,
    cardAccent: 'from-amber-500/8 via-transparent to-transparent',
    iconAccent: 'bg-amber-500/10 text-amber-400 group-hover:bg-amber-500/20',
  },
  {
    Icon: Shield,
    key: 'support',
    tech: ['GitHub Actions', 'Docker', 'Vercel', 'Uptime Robot'],
    timeline: 'Monthly',
    featured: false,
    cardAccent: 'from-indigo-500/8 via-transparent to-transparent',
    iconAccent: 'bg-indigo-500/10 text-indigo-400 group-hover:bg-indigo-500/20',
  },
];
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {SERVICES.map(({ Icon, key, tech, timeline, featured, cardAccent, iconAccent }, i) => (
            <motion.article
              key={key}
              {...inView(0.06 * (i % 2 === 0 ? 0 : 1))}
              className="group relative flex flex-col gap-6 p-7 rounded-2xl border border-border/50 bg-card/30 backdrop-blur-sm overflow-hidden transition-all duration-300 hover:border-primary/30 hover:shadow-glow-sm"
            >
              {/* Hover gradient overlay */}
              <div
                className={`pointer-events-none absolute inset-0 rounded-2xl bg-linear-to-br ${cardAccent} opacity-0 transition-opacity duration-400 group-hover:opacity-100`}
                aria-hidden
              />
              ...
              <div className="relative flex items-start justify-between gap-3">
                ...
                {featured ? (
                  ...
                  <span className="inline-flex items-center px-2.5 py-1 rounded-full border border-border/60 bg-muted/20 text-xs font-semibold text-muted-foreground">
                    {t(`${key}.badge` as Parameters<typeof t>[0])}
                  </span>
                )}
              </div>
              ...
              <div className="relative flex flex-col gap-2">
                <h3 className="text-xl font-bold text-foreground">
                  {ts(`${key}.title` as Parameters<typeof ts>[0])}
                </h3>
                <p className="text-base text-muted-foreground leading-relaxed">
                  {ts(`${key}.description` as Parameters<typeof ts>[0])}
                </p>
              </div>

              {/* ── Feature list ── */}
              <div className="relative flex flex-col gap-3">
                <p className="text-xs font-semibold text-muted-foreground/55 uppercase tracking-[0.13em]">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {tech.map((item) => (
                      <span
                        key={item}
                        className="px-2.5 py-1 text-xs font-medium rounded-lg border border-border/50 bg-primary/6 text-foreground/80"
                      >
                        {item}
                      </span>
                    ))}
