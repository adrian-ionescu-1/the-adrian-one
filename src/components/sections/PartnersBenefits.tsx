...
export function PartnersBenefits() {
  ...
  return (
    <section className="relative border-t border-border/30 py-24 md:py-32 overflow-hidden">
      {/* Subtle glow */}
      <div
        className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-150 h-72 rounded-full bg-primary/7 blur-[100px]"
        aria-hidden
      />
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {BENEFITS.map(({ icon: Icon, accent, bg, titleKey, descKey }, i) => (
            <motion.div
              key={titleKey}
            ...
            >
              {/* Top hover line */}
              <div
                className="pointer-events-none absolute inset-x-0 top-0 h-px bg-linear-to-r from-transparent via-primary/40 to-transparent opacity-0 transition-opacity duration-400 group-hover:opacity-100"
                aria-hidden
              />
