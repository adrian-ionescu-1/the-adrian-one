...
// ─── Static structural config (non-translatable) ─────────────────────────────
...
const PROJECTS_CONFIG: {
  ...
  },
  {
    id: 'greenmarket',
    category: 'ecommerce',
    tech: ['Next.js', 'Sanity', 'Stripe', 'Tailwind CSS'],
  ...
  },
  {
    id: 'lumina',
    category: 'landing',
    tech: ['Next.js', 'Framer Motion', 'Tailwind CSS', 'next-intl'],
...
function FeaturedCard({
  ...
  return (
    <motion.article
      ...
      className="group relative mb-14 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 p-7 sm:p-10 rounded-2xl border border-border/50 bg-card/30 backdrop-blur-sm overflow-hidden transition-all duration-300 hover:border-primary/30 hover:shadow-glow-sm"
    >
      {/* Gradient overlay on hover */}
      <div
        className="pointer-events-none absolute inset-0 bg-linear-to-br from-primary/6 via-transparent to-transparent opacity-0 transition-opacity duration-400 group-hover:opacity-100"
        aria-hidden
      />
      {/* Top glow line */}
      <div
      ...
      {/* ── Left: info ── */}
      <div className="relative flex flex-col gap-5 justify-center order-2 lg:order-1">
        {/* Badges row */}
        <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col gap-1.5 p-4 rounded-xl border border-primary/20 bg-primary/5">
          <p className="text-xs font-bold text-primary/80 uppercase tracking-[0.13em]">
            {labels.challenge}
          </p>
        ...
        {/* Tech stack */}
        <div>
          <p className="text-xs font-semibold text-muted-foreground/55 uppercase tracking-[0.13em] mb-2">
            {labels.techUsed}
          </p>
        ...
        <div className="grid grid-cols-3 gap-3">
          {project.metrics.map(({ value, label }) => (
            <div
              key={label}
              className="flex flex-col items-center gap-1 p-3 rounded-xl border border-border/40 bg-card/50 text-center"
            >
      ...
      <div className="relative flex items-center order-1 lg:order-2">
        {/* Glow behind mockup */}
        <div
          className={`pointer-events-none absolute -inset-4 rounded-2xl bg-linear-to-br ${project.gradient} opacity-15 blur-2xl`}
          aria-hidden
        />
...
function ProjectCard({
  ...
  return (
    <motion.article
      ...
      className="group flex flex-col rounded-2xl border border-border/50 bg-card/30 backdrop-blur-sm overflow-hidden transition-all duration-300 hover:border-primary/30 hover:shadow-glow-sm"
    >
      {/* Mockup area with hover overlay */}
      <div className="relative overflow-hidden">
        <BrowserMockup gradient={project.gradient} />
        {/* Hover overlay — link appears */}
      ...
      <div className="flex flex-col gap-3.5 p-5 flex-1">
        ...
        <div className="flex flex-wrap gap-1.5 mt-auto">
          {project.tech.slice(0, 4).map((item) => (
            <span
              key={item}
              className="px-2 py-0.5 text-xs font-medium rounded-md border border-border/40 bg-primary/5 text-foreground/70"
            >
              {item}
            </span>
          ))}
...
export function PortfolioGrid() {
  ...
  // Build featured project from static config + translations
  ...
  // Build projects array from static config + translations
  ...
  return (
    <section className="relative border-t border-border/30 py-24 md:py-32">
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        {/* Featured project */}
        <FeaturedCard project={featured} labels={featuredLabels} />

        {/* "More projects" label + filter tabs */}
